...
public class AnglePiece extends GenericPiece {
    ...
    {
        return tas.getValue(getEffectiveRadius(lane.getDistanceFromCenter()));
        //return getLength(lane) / (Math.abs(angle) / tas.getValue(getEffectiveRadius(lane.getDistanceFromCenter())));
    }
