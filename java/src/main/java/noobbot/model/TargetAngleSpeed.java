...
public class TargetAngleSpeed {
    ...
    public void calibrate(Position newPosition, Piece currentPiece, SlipAngle slipAngle, double currentSpeed, double currentThrottle) {
        ...
        System.out.println(String.format("P: %2s\tAngle: %4s\tRadius: %4s\tA: %8.6f\tC: %8.6f\tSA: %5.2f\tAS: %5.2f\tTAS: %5.2f\t", currentPiece.getNumber(), currentPiece.getAngle(), currentPiece.getRadius(), slipAngle.getAcceleration(), slipAngle.getSlipChangeVelocity(), slipAngle.getValue(), currentAngleSpeed, targetAngleSpeed));
        if (Math.abs(currentAngleSpeed) > 0 && currentThrottle != 1) {
            if (inCalibration && currentPiece.getAngle() != 0) {
                ...
                if (Math.abs(slipAngle.getAcceleration()) < 0.03 && Math.abs(slipAngle.getSlipChangeVelocity()) < 0.1 && Math.abs(slipAngle.getValue()) < 25) {
